...
RME_ROOT:= $(RME_PATH)
RME_DIR := $(RME_ROOT)/val/src/
SMMU_DIR := $(RME_ROOT)/val/sys_arch_src/smmu_v3
GIC_V3_DIR := $(RME_ROOT)/val/sys_arch_src/gic/v3
GIC_V2_DIR := $(RME_ROOT)/val/sys_arch_src/gic/v2
GIC_DIR  := $(RME_ROOT)/val/sys_arch_src/gic
GIC_ITS_DIR := $(RME_ROOT)/val/sys_arch_src/gic/its

RME_A64_DIR := $(RME_ROOT)/val/src/AArch64/
RME_GIC_DIR := $(RME_ROOT)/val/sys_arch_src/gic/AArch64/
RME_GIC_V3_DIR := $(RME_ROOT)/val/sys_arch_src/gic/v3/AArch64/

CFLAGS    += -I$(RME_ROOT)/val/include
CFLAGS    += -I$(RME_ROOT)/val/
CFLAGS    += -I$(RME_ROOT)/val/sys_arch_src/smmu_v3
CFLAGS    += -I$(RME_ROOT)/val/sys_arch_src/gic/
CFLAGS    += -I$(RME_ROOT)/val/sys_arch_src/gic/its
CFLAGS    += -I$(RME_ROOT)/val/sys_arch_src/gic/v3
CFLAGS    += -I$(RME_ROOT)/val/sys_arch_src/gic/v2
ASFLAGS   += -I$(RME_ROOT)/val/src/AArch64/

DEPS = $(RME_ROOT)/val/include/val_interface.h
...
FILES   := $(foreach files,$(RME_DIR),$(wildcard $(files)/*.c))
...
FILES   += $(foreach files,$(RME_A64_DIR),$(wildcard $(files)/*.S))
FILES   += $(foreach files,$(RME_GIC_DIR),$(wildcard $(files)/*.S))
FILES   += $(foreach files,$(RME_GIC_V3_DIR),$(wildcard $(files)/*.S))
...
$(OBJ_DIR)/%.o: $(RME_A64_DIR)/%.S
	$(CC) $(CFLAGS) $(ASFLAGS) -c -o $@ $< >> $(OUT_DIR)/compile.log 2>&1

$(OBJ_DIR)/%.o: $(RME_GIC_DIR)/%.S
	$(CC)  $(CFLAGS) $(ASFLAGS) -c -o $@ $< >> $(OUT_DIR)/compile.log 2>&1

$(OBJ_DIR)/%.o: $(RME_GIC_V3_DIR)/%.S
	$(CC) $(CFLAGS) $(ASFLAGS) -c -o $@ $< >> $(OUT_DIR)/compile.log 2>&1
