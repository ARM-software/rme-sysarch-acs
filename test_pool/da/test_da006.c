...
#define TEST_NUM (ACS_RME_DA_TEST_NUM_BASE  +  06)
...
payload(void)
{
  ...
  while (instance-- != 0)
  {
      ...
      {
          val_print(ACS_PRINT_ERR, "\n       Failed to lock the device: 0x%lx", req_e_bdf);
          goto test_clean;
      }

      if (val_device_lock(tgt_e_bdf))
      {
          val_print(ACS_PRINT_ERR, "\n       Failed to lock the device: 0x%lx", tgt_e_bdf);
          goto test_clean;
      }
      ...
      if (status)
      {
          val_print(ACS_PRINT_DEBUG, "\n    Putting the devices into unlocked state", 0);
...
test_clean:
      val_print(ACS_PRINT_DEBUG, "\n    Putting the devices back into unlocked state", 0);
      val_device_unlock(req_e_bdf);
      val_device_unlock(tgt_e_bdf);
      val_pcie_disable_tdisp(req_rp_bdf);
      val_pcie_disable_tdisp(tgt_rp_bdf);
