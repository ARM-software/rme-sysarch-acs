...
#define TEST_NAME "da_ide_state_rootport_error"
...
inject_error(uint32_t e_bdf, uint32_t instance)
{
    ...
    {
        val_print(ACS_PRINT_ERR, " Error is not detected", 0);
        return 1;
    }
...
payload()
{
  ...
  while (instance-- != 0) {
    ...
    if (val_pcie_find_capability(e_bdf, PCIE_ECAP, ECID_AER, &aer_offset) != PCIE_SUCCESS) {
        val_print(ACS_PRINT_ERR, " No AER Capability, Skipping for Bdf : 0x%x", e_bdf);
        continue;
    }
    ...
    if (status)
    {
        val_print(ACS_PRINT_ERR, " Failed to get num of Sel stream for BDF: 0x%x", e_bdf);
        test_fails++;
        continue;
    }
    ...
    while (count++ < num_sel_ide_stream_supp)
    {
        ...
        {
            val_print(ACS_PRINT_ERR, " Errror injection failed for BDF: 0x%x", e_bdf);
            test_fails++;
            continue;
        }
        ...
        if (status)
        {
            val_print(ACS_PRINT_ERR, " Failed to get SEL_STR state for BDF: 0x%x", erp_bdf);
            test_fails++;
            continue;
        }
        ...
        {
            val_print(ACS_PRINT_ERR, " SEL_STR is not in Insecure for BDF: 0x%x", erp_bdf);
            test_fails++;
            continue;
        }
        ...
        if (status)
        {
            val_print(ACS_PRINT_ERR, " Failed to disable SEL_STR for BDF: 0x%x", erp_bdf);
            test_fails++;
            continue;
        }
...
uint32_t
da_ide_state_rootport_error_entry(uint32_t num_pe)
{
