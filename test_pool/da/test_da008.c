...
#define TEST_RULE  "RKZBHV, RMYKFH, RGKHSZ"
...
payload(void)
{
  ...
  for (instance = 0; instance < num_exercisers; ++instance)
  {
      ...
      shared_data->shared_data_access[0].addr = va + da_cap_base + RMEDA_CTL2;
      ...
      val_add_mmu_entry_el3((uint64_t)dram_buf_in_virt, (uint64_t)dram_buf_in_virt, ROOT_PAS);
      val_add_mmu_entry_el3((uint64_t)dram_buf_in_virt2, (uint64_t)dram_buf_in_virt2, ROOT_PAS);
      ...
      shared_data->shared_data_access[0].addr = (uint64_t)dram_buf_in_virt;
      shared_data->shared_data_access[0].access_type = READ_DATA;
      shared_data->shared_data_access[1].addr = (uint64_t)dram_buf_in_virt2;
      shared_data->shared_data_access[1].access_type = READ_DATA;
      val_pe_access_mut_el3();
      int data1 = shared_data->shared_data_access[0].data;
      int data2 = shared_data->shared_data_access[1].data;

      val_print(ACS_PRINT_DEBUG, "\n    dram_buf_in_virt data: 0x%x", data1);
      val_print(ACS_PRINT_DEBUG, "\n    dram_buf_in_virt2 data: 0x%x", data2);
