...
#define TEST_NUM (ACS_RME_DA_TEST_NUM_BASE  +  04)
...
payload(void)
{
  ...
  while (tbl_index < bdf_tbl_ptr->num_entries)
  {
      ...
      if (dp_type == RP)
      {
          ...
          {
              val_print(ACS_PRINT_ERR, "\n       PCIe IDE Capability not present ", 0);
              continue;
          }
          ...
          {
              val_print(ACS_PRINT_ERR, "\n       TEE limited str not supported for BDF: %x", bdf);
              test_fails++;
          }
          ...
          while (count <= num_sel_ide_stream_supp)
          {
              ...
              {
                  val_print(ACS_PRINT_ERR, "\n       Addr asso reg blk is < 3 for BDF: %x", bdf);
                  test_fails++;
                  continue;
              }
