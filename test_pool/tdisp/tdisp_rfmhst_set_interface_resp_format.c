...
#define TEST_NAME "tdisp_rfmhst_set_interface_resp_format"
#define TEST_DESC "Validate SET_INTERFACE_RESP format"
#define TEST_RULE "RFMHST"

static uint32_t
tdisp_rfmhst_validate_set_interface_resp(uint32_t bdf,
                                         ...
                                         uint32_t response_size)
{
  ...
  /* SET_INTERFACE_RESP is expected to contain just the Arm VDM + req/resp headers. */
  if (response_size != VAL_TDISP_ARM_VDM_BASE_SIZE)
  {
    val_print(ACS_PRINT_ERR, " Unexpected SET_INTERFACE_RESP size for 0x%x", bdf);
    return ACS_STATUS_FAIL;
  }

  rsp_msg_type = response[VAL_TDISP_HDR_MSG_TYPE_OFF];
  /* SET_INTERFACE_RESP must use VDM_RSP MessageType. */
  ...
  {
    val_print(ACS_PRINT_ERR, " SET_INTERFACE_RESP type mismatch for 0x%x", bdf);
    return ACS_STATUS_FAIL;
  }
...
payload(void)
{
  ...
  for (root_index = 0u; root_index < cxl_tbl_ptr->num_entries; ++root_index)
  {
    ...
    status = val_tdisp_get_interface_state(&spdm_context, session_id, &interface_id, &tdi_state);
    if (status != ACS_STATUS_PASS)
      goto cleanup;
    ...
    /* Capture the raw SET_INTERFACE response for format validation. */
    response_size = (uint32_t)sizeof(response);
    /* Send SET_INTERFACE and validate the response framing. */
    ...
    /* Validate the SET_INTERFACE_RESP format per rule RFMHST. */
    status = tdisp_rfmhst_validate_set_interface_resp(bdf, response, response_size);
    if (status != ACS_STATUS_PASS)
      test_fail++;
...
uint32_t
tdisp_rfmhst_set_interface_resp_format_entry(void)
{
  uint32_t num_pe;
  uint32_t status;
