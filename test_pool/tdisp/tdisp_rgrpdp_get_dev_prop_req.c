...
#define TEST_NAME "tdisp_rgrpdp_get_dev_prop_req"
#define TEST_DESC "Validate GET_DEV_PROP_REQ semantics"
#define TEST_RULE "RGRPDP"

static uint32_t
tdisp_rgrpdp_validate_get_dev_prop_success_rsp(uint32_t bdf,
                                               ...
                                               uint32_t response_size)
{
  ...
  /* Validate the response envelope and that the Arm message is GET_DEV_PROP_RESP. */
  ...
  if (rsp_arm_type != VAL_TDISP_ARM_MSG_GET_DEV_PROP_RESP)
  {
...
static uint32_t
tdisp_rgrpdp_send_get_dev_prop_with_interface_id(uint32_t bdf,
                                                 ...
                                                 const pci_tdisp_interface_id_t *interface_id)
{
  ...
  /* Send Arm VDM GET_DEV_PROP over SPDM and validate we get a success response. */
  ...
  {
    val_print(ACS_PRINT_ERR, " GET_DEV_PROP send failed for 0x%x", bdf);
    return status;
  }

  return tdisp_rgrpdp_validate_get_dev_prop_success_rsp(bdf, response, response_size);
}
...
payload(void)
{
  ...
  for (root_index = 0u; root_index < cxl_tbl_ptr->num_entries; ++root_index)
  {
    ...
    /* Baseline: confirm the device accepts GET_DEV_PROP with a valid interface id. */
    status = tdisp_rgrpdp_send_get_dev_prop_with_interface_id(bdf,
                                                              &spdm_context,
                                                              session_id,
                                                              &interface_id_valid);
    if (status == ACS_STATUS_SKIP)
    ...
    /*
     * Rule RGRPDP: GET_DEV_PROP_REQ does not apply to a specific INTERFACE_ID and
