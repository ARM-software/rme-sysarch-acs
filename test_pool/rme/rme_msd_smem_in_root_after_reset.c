...
#define TEST_NAME  "rme_msd_smem_in_root_after_reset"
#define TEST_DESC  "Verify that MSD SMEM is in ROOT PAS after reset        "
#define TEST_RULE  "RCSSDG"
...
reset_done:
  ...
  if (root_smem_avaialbile)
    PA = root_smem_avaialbile;
  else {
...
uint32_t
rme_msd_smem_in_root_after_reset_entry(uint32_t num_pe)
{
