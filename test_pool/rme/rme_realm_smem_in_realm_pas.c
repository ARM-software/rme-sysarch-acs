...
#define TEST_NAME  "rme_realm_smem_in_realm_pas"
#define TEST_DESC  "Verify that Realm SMEM is in realm PAS                 "
#define TEST_RULE  "RCMMCZ/RZQQSQ"

/**
 * @brief  The test validates that the Realm SMEM can always be accessed through REALM PAS only
 * 1. Access the Realm SMEM using all access PAS
 * 2. Observe that only Realm access PAS is successful
**/
static
void payload(void)
{
  ...
  uint64_t pas_list[4] = {REALM_PAS, NONSECURE_PAS, SECURE_PAS, ROOT_PAS};
  ...
  if (realm_smem_available)
    PA = realm_smem_available;
  else {
    PA = val_get_free_pa(size, size);
    val_add_gpt_entry_el3(PA, GPT_REALM);
  }

  VA = val_get_free_va(NUM_PAS * NUM_SMEM_REGN * size);
  security_state = REALM_PAS;
  attr = LOWER_ATTRS(PGT_ENTRY_ACCESS | SHAREABLE_ATTR(NON_SHAREABLE) | PGT_ENTRY_AP_RW);

  for (int pas_cnt = 0; pas_cnt < 4; ++pas_cnt)
  {
      val_print(ACS_PRINT_TEST, " Accessing Realm SMEM with PAS = %lld", pas_list[pas_cnt]);
      ...
      if (security_state == pas_list[pas_cnt]) {
        ...
        val_print(ACS_PRINT_INFO, " The data read when res pas != acc pas is 0x%lx", rd_data1);
...
uint32_t
rme_realm_smem_in_realm_pas_entry(uint32_t num_pe)
{
