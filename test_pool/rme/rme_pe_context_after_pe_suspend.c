...
#define TEST_NAME  "rme_pe_context_after_pe_suspend"
...
isr()
{
  ...
  irq_received = 1;
  val_print(ACS_PRINT_TEST, " System timer interrupt received", 0);
}
...
payload()
{
  ...
  if (!ns_timer) {
      val_print(ACS_PRINT_ERR, " No non-secure systimer implemented", 0);
...
uint32_t
rme_pe_context_after_pe_suspend_entry(uint32_t num_pe)
{
