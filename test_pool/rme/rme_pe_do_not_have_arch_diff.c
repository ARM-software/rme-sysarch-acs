...
return_reg_value(uint32_t reg, uint8_t dependency)
{
  ...
  switch (dependency)
  {
    ...
    default:
        val_print(ACS_PRINT_ERR, " Unknown dependency = %d ", dependency);
        return 0x0;
  }
...
/*
 * @brief  The test validates that application PEs do not have any architectural differences.
 * 1. Select the correct cache in CSSELR and store the register value of CCSIDR_EL1 in
 *    cache_list[] for each cache level till maximum cahe level.
 * 2. Read the register values and store them in rd_data_array[].
 * 3. Start executing the id_reg_check payload in the secondary PEs.
 * 4. Execute the first step for the register values of the secondary PEs and compare them with
 *    the values stored in cache_list[] and rd_data_array[] of the primary PE and return the result.
 * 5. Execute the same payload in all the other secondary PEs.
 */
static
void
payload(uint32_t num_pe)
{
  ...
  while (i < MAX_CACHE_LEVEL) {
      ...
      if (reg_read_data & ((0x7) << (i * 3))) {
         ...
         val_print(ACS_PRINT_DEBUG, " Cache size read is %x ", cache_list[i]);
      }
      i++;
  }
  ...
  for (i = 0; i < num_pe; i++) {
      if (i != my_index) {
          ...
          if (timeout == 0) {
              val_print(ACS_PRINT_ERR, " **Timed out** for PE index = %d", i);
              val_set_status(i, "FAIL", 02);
              return;
          }
          ...
          if (IS_TEST_FAIL(val_get_status(i))) {
              ...
              val_print(ACS_PRINT_ERR, " Current PE value = 0x%llx",
                        rd_data_array[array_index] & (~reg_list[array_index].reg_mask));
...
uint32_t
rme_pe_do_not_have_arch_diff_entry(uint32_t num_pe)
{
