...
#define TEST_NAME "legacy_tz_enable_after_reset"
...
payload()
{
  ...
  if (rme_impl_smmu && rme_impl_pe) {
    ...
    if (rme_impl_pe)
      val_print(ACS_PRINT_TEST, " The RME implementation bit of PE is set\
                      even after enabling LEGACY_TZ_EN", 0);
    if (rme_impl_smmu)
      val_print(ACS_PRINT_TEST, " The RME implementation bit of SMMU is set\
                      even after enabling LEGACY_TZ_EN", 0);
    ...
    if (!rme_impl_pe)
      val_print(ACS_PRINT_ERR, " The RME implementation bit of PE is reset to RES0 ", 0);

    if (!rme_impl_smmu)
      val_print(ACS_PRINT_ERR, " The RME implementation bit of SMMU is reset to RES0", 0);
...
uint32_t
legacy_tz_enable_after_reset_entry(uint32_t num_pe)
{
  num_pe = 1;
