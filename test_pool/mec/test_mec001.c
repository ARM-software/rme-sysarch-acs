...
#define TEST_NUM  (ACS_RME_MEC_TEST_NUM_BASE + 1)
...
payload2(void)
{
  ...
  if (common_mecidw == INVALID_MECIDW) {
      val_print(ACS_PRINT_ERR, "\nFailed to determine common MECID width", 0);
...
mec001_entry(uint32_t num_pe)
{
  ...
  if (!pe_mecidw) {
      val_print(ACS_PRINT_ERR, "Failed to allocate shared memory\n", 0);
      val_report_status(pe_index, ACS_STATUS_FAIL, TEST_RULE);
