...
#define TEST_NUM  (ACS_RME_DPT_TEST_NUM_BASE + 7)
#define TEST_DESC  "TDI DPT Check - P2P Same RP - Invalid Access           "
...
payload(void)
{
  ...
  for (instance = 0; instance < num_exercisers; ++instance)
  {
      ...
      {
          val_print(ACS_PRINT_DEBUG, "\n       No Exerciser found on the same RP, BDF 0x%x", bdf);
          continue;
      }
      ...
      {
          val_print(ACS_PRINT_ERR, "\n       Bar index not found for bdf: 0x%x", tgt_e_bdf);
          continue;
      }
      ...
      if (val_pcie_find_capability(rp_bdf, PCIE_ECAP, ECID_ACS, &cap_base) == PCIE_SUCCESS)
      {
          ...
          val_pcie_write_cfg(rp_bdf, cap_base + ACSCR_OFFSET, acs_data | (1 << ACSUFE_SHIFT));
      }
      ...
      val_print(ACS_PRINT_DEBUG, "\n The translated address obtained is: 0x%llx", translated_addr);
